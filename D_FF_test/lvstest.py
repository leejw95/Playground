...
class LVStest :
    ...
    def LVSchecker(self) :
        ...
        commandlines3 = "cd {0}; sed -i '11s,.*,LAYOUT PATH  \"{0}/{1}.calibre.db\",' {2}; sed -i '12s,.*,LAYOUT PRIMARY \"{1}\",' {2}; sed -i '15s,.*,SOURCE PATH \"{3}/{1}.sp\",' {2}; sed -i '16s,.*,SOURCE PRIMARY \"{1}\",' {2}; sed -i '21s,.*,LVS REPORT \"{1}.lvs.report\",' {2}; sed -i '46s,.*,ERC RESULTS DATABASE \"{1}.erc.results\" ,' {2}; sed -i '47s,.*,DRC SUMMARY REPORT \"{1}.erc.summary\" REPLACE HIER,' {2}"
        ...
        commandlines33 = f"cd {self.WorkDir}; rm {self.cellname}.lvs.report; rm {self.cellname}.erc.results; rm {self.cellname}.erc.summary"
        ...
        commandlines4 = "cd {0}; source setup.cshrc; calibre -spice {0}/{3}.sp -turbo -lvs -hier -nowait {1}/{2}"
        stdin, stdout, stderr = ssh.exec_command(commandlines4.format(self.WorkDir, self.DRCrunDir, LVSfile, self.cellname))
        stdout.read()
