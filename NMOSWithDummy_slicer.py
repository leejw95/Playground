...
class _NMOS(StickDiagram._StickDiagram):
    ...
    def _CalculateNMOSDesignParameter(self, _NMOSNumberofGate=None, _NMOSChannelWidth=None, _NMOSChannellength=None, _NMOSDummy=False, _SLVT=False):
        # print '#########################################################################################################'
        ...
        ################################## Poly layer to avid DRC by Dummy Layer ####################################################
        self._DesignParameter['_POLayerGate'] = self._BoundaryElementDeclaration(_Layer=DesignParameters._LayerMapping['POLY'][0], _Datatype=DesignParameters._LayerMapping['POLY'][1], _XYCoordinates=[], _XWidth=400, _YWidth=400)
        self._DesignParameter['_POLayerGate']['_XWidth'] = _NMOSChannellength
        self._DesignParameter['_POLayerGate']['_YWidth'] = 120
        _LengthNMOSBtwPO = _DRCObj.DRCPolygateMinSpace(_DRCObj._CoMinWidth + 2 * _DRCObj._PolygateMinSpace2Co) + self._DesignParameter['_POLayer']['_XWidth']
        tmp = []
        for i in range(0, _NMOSNumberofGate):
            if (_NMOSNumberofGate % 2) == 0:
                _xycoordinatetmp = [_XYCoordinateOfNMOS[0][0] - (_NMOSNumberofGate / 2 - 0.5) \
                                    * _LengthNMOSBtwPO + i * _LengthNMOSBtwPO, _XYCoordinateOfNMOS[0][1] + self._DesignParameter['_POLayer']['_YWidth']/2 + self._DesignParameter['_POLayerGate']['_YWidth']/2]
            elif (_NMOSNumberofGate % 2) == 1:
                _xycoordinatetmp = [_XYCoordinateOfNMOS[0][0] - (_NMOSNumberofGate - 1) / 2 \
                                    * _LengthNMOSBtwPO + i * _LengthNMOSBtwPO, _XYCoordinateOfNMOS[0][1] + self._DesignParameter['_POLayer']['_YWidth']/2 + self._DesignParameter['_POLayerGate']['_YWidth']/2]
            tmp.append(_xycoordinatetmp)
        self._DesignParameter['_POLayerGate']['_XYCoordinates'] = tmp
        ...
        if _NMOSDummy == True:
            ...
            if (_NMOSNumberofGate % 2) == 0: # When the number of finger is even
                _xycoordinatetmp_dummy = [
                                   [_XYCoordinateOfNMOS[0][0] - ( _NMOSNumberofGate / 2 - 0.5) *  _LengthNMOSBtwPO + 0 *  _LengthNMOSBtwPO - _DRCObj.DRCPolygateMinSpace(_DRCObj._CoMinWidth +  _DRCObj._PolygateMinSpace2Co + _DRCObj._CoMinEnclosureByOD + _DRCObj._PolygateMinSpace2OD ) - (float(self._DesignParameter['_PODummyLayer']['_XWidth'])/2 + float(self._DesignParameter['_POLayer']['_XWidth'])/2) ,  _XYCoordinateOfNMOS[0][1] - float(self._DesignParameter['_POLayer']['_YWidth'])/2 + float(self._DesignParameter['_PODummyLayer']['_YWidth'])/2+25],
        ...
        if _NMOSChannelWidth < 258:
            self._DesignParameter['_PODummyLayer']['_XYCoordinates'] = [[_xycoordinatetmp_dummy[0][0], _xycoordinatetmp_dummy[0][1]], [_xycoordinatetmp_dummy[1][0], _xycoordinatetmp_dummy[1][1]]]
...
###############################################################################################################################################################################
        ...
        if _SLVT == True:
            ...
            self._DesignParameter['_SLVTLayer']['_XWidth'] = self._DesignParameter['_ODLayer']['_XWidth'] + 60*2
            self._DesignParameter['_SLVTLayer']['_YWidth'] = self._DesignParameter['_ODLayer']['_YWidth'] + 81*2
